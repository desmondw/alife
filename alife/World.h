...
class World
{
	...
	public:
		...
		void creatureMove(int x, int y, int newX, int newY);
		void creatureEat(int x, int y, int foodX, int foodY);
		bool testTileExists(int x, int y);
		void fillPredatorEscapeLists(int creatureX, int creatureY, int x, int y, std::vector<std::vector<int>>& predatorList, std::vector<std::vector<int>>& escapeList);
		bool testPredatorClose(int x, int y);
